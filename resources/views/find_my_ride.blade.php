...
$p=round((distance($ride->src_lat,$ride->src_long,$ride->des_lat,$ride->des_long)*1.2),2);
@endphp
...
<div class="container-fluid" id="my">
    ...
    @if(count($reqs)>0)
        <div class=row style="background-color:lavenderblush;font-size:24px;color:Purple;">
                <div class="col-md-12">
                <br />
                ...
                <table class="table table-bordered">
                    ...
                    @endphp
                        ...
                        @foreach ($reqs as $row)
                            ...
                            @if((($row->ladies==1 and auth()->user()->gender=="Female") or $row->ladies==0) and $row->no_of_seats>0)
                            <tr>
                                <td> {{$row->source}}</td>
                                <td> {{$row->des}}</td>
                                <td> {{$row->date}}</td>
                                ...
                                <form method='POST' action='/confirm'>
                                    @csrf
                                    <input type="hidden" value={{$row->id}} name="off_id">
                                    <input type="hidden" value={{$ride->id}} name="req_id">
